...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Pre-populate blog topics
    const topic = jQuery('#topic');
    topic.select2();
    const baseUrl = window.location.origin;
    const token = localStorage.getItem('authToken');
    const blogId = '{{$id}}'; // Assuming the blog ID is passed to the page
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        $.each(response.data, function(index, item) {
          if (item.topic) {
            var option = '<option value="' + item.topic + '">' + item.topic + '</option>';
            topic.append(option);
          }
        });
      },
      complete: function() {
        // Pre-fill form with existing blog data (for editing)
        $.ajax({
          url: baseUrl + '/fetchBlogDetailsApi/' + blogId,
          method: 'GET',
          dataType: 'json',
          headers: {
            'Authorization': 'Bearer ' + token,
          ...
          success: function(response) {
            if (response.status === '1') {
              $('#topic').val(response.data.topic).trigger('change');
              $('#heading').val(response.data.heading);
              editorInstance.setData(response.data.content);

              if (response.data.image_url) {
                uploadedImage.src = response.data.image_url;
                uploadedImage.style.display = 'block';
                imageLabel.style.display = 'none';
              }

              if (response.data.video_url) {
                uploadedVideo.src = response.data.video_url;
                uploadedVideo.style.display = 'block';
                videoLabel.style.display = 'none';
              }

              if (response.data.pdf_name) {
                pdfName.textContent = response.data.pdf_name;
                pdfName.style.display = 'block';
                pdfLabel.style.display = 'none';
              }
            }
...
<script>
  $(document).ready(function() {
    ...
    function submitForm(status) {
      ...
      $('.msg_err').text(''); // Clear error messages
      ...
      // Append blog ID and status
      formData.append('status', status);
      formData.append('blogId', '{{$id}}'); // Include the blog ID for updates

      // AJAX request
      $.ajax({
        url: baseUrl + '/updateBlogsForAdminApi', // Update the blog
        type: 'POST',
        data: formData,
        ...
        beforeSend: function() {
          let message = status === 'save' ? 'Saving' : 'Publishing';
        ...
        error: function(xhr) {
          if (xhr.status === 422) {
            ...
            $.each(errors, function(field, messages) {
              $(`#${field}Error`).text(messages[0]); // Display the error message
            });
          }
        },
        complete: function() {
